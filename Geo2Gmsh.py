...
def create_surface(
    ...
    data = np.loadtxt(f"data/layers/{file_name}", skiprows=1)
    id = (surf_id-1) * 10
...
def add_well(
    ...
    well_txt = np.loadtxt(f"data/wells/{file_name}", skiprows=1, dtype=float)
...
def add_fault(
    ...
    fault_txt = np.loadtxt(f"data/faults/{file_name}", skiprows=1)
